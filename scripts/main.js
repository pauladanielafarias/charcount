$(document).ready(function () {
    ...
    $("#textCharcount").on("input", (event) => {
        ...
        //cuenta las palabras con la funcion countWords()
        let live_words = countWords(input_value);

        //cuenta los numeros con la funcion countNumbers()
        let live_numbers = countNumbers(input_value);

        //inserta en el html en #live_result
        $("#live_result").html(
            `<div class="d-flex justify-content-center">\
                <h1 class="characters mx-4">Characters: <span id="live_characters"> ${live_characters}</span></h1>\
                <h1 class="words mx-4">Words: <span id="live_words"> ${live_words}</span></h1>\
                <h1 class="words mx-4">Numbers: <span id="live_numbers"> ${live_numbers}</span></h1>\
            </div>\
            `
        );
    ...
    let word_regex = /([A-Z]|[a-z])+(\.|!|\?|,| |\n|)/g;
    // function that counts the words of a given text
    const countWords = (input_value) => {
    ...
    // function that counts the numbers of a given text
    const countNumbers = (input_value) => {
